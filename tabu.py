...
def constraints(player_to_add, player_to_remove, club_constraints, budget):
    if club_constraints[player_to_add.club] < 3 and budget >= (player_to_add.value - player_to_remove.value):
        return True
    return False
...
if __name__ == '__main__':
    ...
    search = input("Please input 't' for tabu or 's' for simulated annealing search: ")
    csv_file = 'instance' + str(no_instance) + '.csv'
    slicing_step = 3 if no_instance == 1 else 4       # explanation on line 123 !
    ...
    if search == 't':
        tabu_list = Counter()
        tabu_tenure = 100
        incumbent_points = total_points
        incumbent_solution = best_eleven.copy()
        best_team = my_team.copy()
        valid_budget = starting_budget

        no_iter_without_improvement = 0
        while no_iter_without_improvement < 10000:
            player_replacements = []

            for i in range(11):
                player_to_remove = best_eleven[i]
                position = player_to_remove.pos
                tabu_list[player_to_remove] = tabu_tenure
            ...
            for player in tabu_list.keys():
                if player != player_to_remove and tabu_list[player] > 0:
                    tabu_list[player] -= 1
            ...
            if total_points > incumbent_points:
                incumbent_points = total_points
                incumbent_solution = best_eleven.copy()
                best_team = my_team.copy()
                valid_budget = starting_budget
                ...
                no_iter_without_improvement += 1
            print("Total points: {0}".format(total_points))
            print("Incumbent solution: {0}".format(incumbent_points))
            # print("Current budget: {0}".format(starting_budget))
    ...
    #     7. SIMULATED ANNEALING     #
        ...
        for i in range(11):
            ...
            while temperature >= 0.01:
                ...
                player = random.choice(all_players_dict[position])
                if clubs_constraints[player.club] < 3 and player not in my_team and starting_budget >= (player.value - player_to_remove.value):
                    point_difference = player.points - player_to_remove.points
                    prob = 1 if point_difference > 0 \
                        else np.exp(point_difference / temperature)
                    if prob >= np.random.random():
                        # print(player)
                        # print(prob)
                        # print(temperature)

                        best_eleven[i] = player
                        my_team.remove(player_to_remove)
                        my_team.add(player)
                        clubs_constraints[player_to_remove.club] -= 1
                        clubs_constraints[player.club] += 1
                        starting_budget -= (player.value - player_to_remove.value)
                        ...
                        print("Incumbent solution: {0}".format(incumbent_points))
                        print("Current budget: {0}".format(starting_budget))
                        print("Player {0} replaced by player {1}".format(player_to_remove, player))

                    # temperature /= (1 + beta*temperature)
                    temperature *= alpha



    #  8. PRINT FINAL CHOSEN FORMATION, PLAYERS' IDS AND INCUMBENT SOLUTION

    best_eleven = incumbent_solution
    print_pretty_formation(best_eleven, best_formation)
    ...
    print("Total points: ", incumbent_points)
    print("Valid remaining budget: ", starting_budget)
